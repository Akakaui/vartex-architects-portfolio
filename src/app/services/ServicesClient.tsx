...
export default function ServicesClient({ initialPage = "index" }: { initialPage?: "index" | "architecture" | "interior" }) {
    ...
    useEffect(() => {
        ...
        // Native touch scrolling is the priority on phones and tablets. The former
        // mobile ScrollTrigger callbacks changed classes while the user was dragging,
        // which made the service pages feel like they required a second swipe.
        if (reducedMotion || touchLayout) return;

        const ctx = gsap.context(() => {
            gsap.utils.toArray<HTMLElement>(".service-reveal:not(.service-card):not(.service-image-stage)").forEach((element, index) => {
                gsap.from(element, {
                    y: 28,
                    duration: 0.65,
